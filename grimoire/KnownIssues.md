...
- Build deps are installed by the `desktop/quickshell` role but the build itself is fragile
- QML files are in `config/quickshell/` and ready; blocked on stable build
- Waybar is active in the meantime
- Status: QML authored, build stabilization needed
