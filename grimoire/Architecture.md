...
Layer 3 — desktop/
            ...
            v4l2/
            network-applet/
            wayland/                 (Wayland session deps)
            shell/zsh/
            shell/bash/
Infra    — snapper/                  (cross-cutting, Btrfs snapshots)
           runtime/                  (cross-cutting, XRT/CUDA/ROCm)
           monitoring/               (cross-cutting, btop/htop/nvtop/powertop)
```
...
│   ├── nvidia         (akmod-nvidia, PRIME offload)
│   ├── asusctl        (asusd daemon)
│   ├── npu            (XDNA kernel module check, ONNX runtime)
│   ├── audio          (pipewire + wireplumber)
│   ├── firmware       (fwupd)
│   ├── power          (power-profiles-daemon, thermald)
│   └── supergfx       (supergfxctl + switcheroo-control)
│
├── hardware/x64/generic              (conditional: generic_x64 group)
│
├── runtime                           (cross-cutting, all hosts)
│   └── xrt.yml        (AMD XRT/XDNA2 — conditional on npu.vendor == "amd")
│
├── monitoring                        (cross-cutting, all hosts)
│   ├── sensors.yml    (htop, btop, nvtop, lm_sensors)
│   └── powertop.yml   (powertop + auto-tune systemd unit)
│
...
│   handler: dracut
│
├── boot/session_manager              (SDDM, wayland-sessions/hyprland.desktop)
│   handler: systemctl restart sddm
│
├── snapper                           (btrfs-only: config, timeline timer, grub-btrfs)
│
└── desktop/
    ├── wayland        (Wayland session packages and env)
    ├── hyprland       (packages, symlink config/hypr → ~/.config/hypr,
    │                   template machine.conf)
    ├── quickshell     (build from source, symlink config/quickshell)
    ├── terminal       (kitty, nerd fonts)
    ├── waybar         (packages, config)
    ├── notifications  (mako)
    ├── launcher       (fuzzel)
    ├── screenshot     (grim, slurp, scripts)
    ├── swayosd        (volume/brightness OSD)
    ├── v4l2           (v4l2loopback, virtual camera)
    ├── network-applet (nm-applet)
    ├── shell/zsh      (zinit, p10k, ZDOTDIR, symlink config/shell/zsh)
        ...
        └── Kernel loads
              └── Plymouth ← boot/plymouth  void bg + magenta RaBbLE text + spinner
                    └── SDDM ← boot/session_manager  QML theme, Wayland-native greeter
                          └── Hyprland ← desktop/hyprland  config/ + machine.conf
...
| OSD | swayosd | Volume/brightness overlay | — |
...
All user configs live as static files in `config/` and are deployed by Ansible (symlinked or copied) into `~/.config/`. Edit the repo file — `RaBbLE-OS-dotctl.sh` keeps `~/.config/` in sync without a full Ansible run.
...
| `config/shell/bash/aliases.sh` | `~/.bash_aliases` |
...
**Not symlinked — machine-local, Ansible-templated or copied:**

| Generated path | Source |
|---|---|
| `~/.config/hypr/machine.conf` | `roles/desktop/hyprland/templates/hyprland-machine.conf.j2` |
...
| `/etc/supergfxd.conf` | `roles/hardware/x64/asus_proart_p16/files/supergfxd.conf` (static) |
